...
TEST_CASE("image_algos_test - FFTTest")
{
    ...
    // transform
    FFT<double> fft( im.size() );
    image_cf output{ fft.transform( im, direction::FORWARD ) };

    REQUIRE( save_to_file( "fft-output.pgm", image_gf( output ) ) );
    ...
    // inverse transform
    image_gf g{ real( fft.transform( output, direction::REVERSE ) ) };
...
TEST_CASE("image_algos_test - FFT two real images")
{
    ...
    // transform
    FFT<double> fft( a.size() );
    auto [a_fft, b_fft] = fft.transform_real( a, b, direction::FORWARD );

    REQUIRE( save_to_file( "fft-real-output-a.pgm", image_gf( a_fft ) ) );
    REQUIRE( save_to_file( "fft-real-output-b.pgm", image_gf( b_fft ) ) );

    // two peaks; relies on quadrants being unswapped i.e. zero
    // frequency is at edges
    auto checker =
        [](const image_cf& im, point2<uint32_t> a, point2<uint32_t> b)
        {
...
{
    _REQUIRE_THROWS_MATCHES( FFT<double>( { 512, 400 } ),
                             std::runtime_error,
                             ContainsSubstring( "power of 2"s, CaseSensitive::No ) );
    _REQUIRE_THROWS_MATCHES( FFT<double>( { 400, 512 } ),
                             std::runtime_error,
                             ContainsSubstring( "power of 2"s, CaseSensitive::No ) );
}
...
TEST_CASE("image_algos_test - cross_correlation_test")
{
    ...
    // prepare & correlate
    FFT<double> fft( view_a.size() );
    image_gf output{ fft.cross_correlate( view_a, view_b ) };
...
TEST_CASE("image_algos_test - auto_correlation_test")
{
    ...
    // combine
    image_gf data{ view_a.width(), view_a.height() };
    data = view_a + view_b;
    REQUIRE( save_to_file( "auto-correlate-input.pgm", data) );

    // prepare & corrrelate
    FFT<double> fft( data.size() );
    image_gf output{ fft.auto_correlate( data ) };
...
TEST_CASE("image_algos_test - fft_real_test")
{
    ...
    REQUIRE( save_to_file( "fft_corr_a_fourier.pgm", image_gf{ output }) );
    ...
    REQUIRE( save_to_file( "fft_corr_a_conj.pgm", image_gf{ output }) );

    // inverse
    output = fft.transform( image_gf{ output }, direction::REVERSE );
    swap_quadrants( output );

    // write output
    REQUIRE( save_to_file( "fft_corr_a_output.pgm", image_gf{ output }) );
}
